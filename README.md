...
==================================================
📦 Имя файла:    NetTie-2_SMD_D1-0.65_D2-3.00_L-3.80mm.kicad_mod
🔧 Тип монтажа:  SMD
...
Полученный `.kicad_mod` кладётся в библиотеку (`.pretty`-каталог) и используется
как обычный footprint для символа Net-Tie.
...
* `exclude_from_pos_files exclude_from_bom allow_missing_courtyard` —
  футпринт не попадает в BOM и pos-файлы, суд по courtyard не выносится;
...
2. Если нет, создать кастомную библиотеку посадочных мест. Например, `.../KiCad/Libraries/Custom_NetTies.pretty`
  ...
  Это директория, но её надо создавать именно в редакторе посадочных мест, иначе она не зарегистрируется. И обязательно шаблон &lt;Name&gt;.pretty
...
5. Если всё устраивает, нажать «Сохранить»

6. Будет предложено выбрать библиотеку. Выбираем созданную `Custom_NetTies`

Посадочным местом можно пользовать.

*Обратите внимание*: По умолчанию пады посадочного места открыты (открытые пятачки меди!) Поэтому, если делаете тип `SMD` лучше добавить ключ `--tented`.
