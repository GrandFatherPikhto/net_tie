...
```
2. Если радиусы падов равны, используем ключ `-d` или `-d1` и `-d2` равные между собой:

```bash
(.venv) PS D:\Projects\Python\nettie_generator> python kicad_nettie_cli.py -v 12 -i 1.0 -d 0.65 -t smd --tented                                                                        
d1 = 0.65, d2=0.65
🔥 Смузи-парабола готова для KiCad 10!
==================================================
📦 Имя файла:    NetTie-2_SMD_D1-0.65_D2-0.65_L-1.45mm_Tented.kicad_mod
🔧 Тип монтажа:  SMD
📏 Физическая L: 1.45 мм
⚡ Узкое горло:  0.600 мм (IPC-минимум 0.300 × 2)
🌡  Непрерывно:   1.7 А при ΔT=10°C
💥 Плавление:    6 А/1с, 60 А/10мс, 602 А/100мкс
↔️  Зазор пад-пад: 0.150 мм (IPC-2221B ×1.5)
🎭 Маска:        закрыта (tented)
==================================================
💾 D:\Projects\Python\nettie_generator\NetTie-2_SMD_D1-0.65_D2-0.65_L-1.45mm_Tented.kicad_mod
```

3. Если библиотеки, в которую собираемся добавить `NetTie`, создаём кастомную библиотеку посадочных мест. Например, `.../KiCad/Libraries/Custom_NetTies.pretty`
...
| `-d` | — | Диаметры падов (1) и (2) совпадают, мм|
...
| `--no-courtyard` |—| Не формировать &laquo;дворик&raquo; (`CourtYard`) для `NetTie` (не рекомендуется, когда дистанции (`clearence`) накладываются друг на друга|
...
Можно определить правило для всех элементов, начинающихся с `NT*`
...
* IPC-2221 рассчитан на длинную дорожку; короткое горло между двумя медными
  ...
  ставьте осознанно.
* Паяльная маска — не изоляционный барьер. Длину пути утечки (это кратчайшее расстояние между двумя проводниками, измеренное по поверхности изолирующего материала `creepage`) считайте так, будто её нет.
